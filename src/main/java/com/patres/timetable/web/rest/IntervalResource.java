...
public class IntervalResource {
    ...
    /**
     * SEARCH  /_search/intervals?query=:query : search for the interval corresponding
     * to the query.
     *
    ...
    @Timed
    public ResponseEntity<List<IntervalDTO>> searchIntervals(@RequestParam String query, @ApiParam Pageable pageable) {
        log.debug("REST request to search for a page of Intervals for query {}", query);
        Page<IntervalDTO> page = intervalService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/intervals");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
